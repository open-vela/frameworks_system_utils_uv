  ...
  if (mode) {
    ...
    for (i = 0; i < sizeof(crypto_mode) / sizeof(crypto_mode[0]); i++) {
      for (j = 0; j < sizeof(padding_mode) / sizeof(padding_mode[0]); j++) {
        ...
        if (uv_aes_set_key_base64(&ctx, MBEDTLS_ENCRYPT, key,
            ctx.aes_context.cipher_info->key_bitlen) != 0) {
          goto testfail;
        }

        if (ctx.aes_context.cipher_info->iv_size != 0 &&
            uv_aes_set_iv_base64(&ctx, iv, 0, ctx.aes_context.cipher_info->iv_size) != 0) {
          goto testfail;
        }

        if(uv_aes_encrypt_base64(&ctx, pstr, strlen((const char *)pstr),
                                  pstrbuff, sizeof(pstrbuff), &pstrbufflen) != 0) {
          goto testfail;
        }
    ...
    for (i = 0; i < sizeof(crypto_mode) / sizeof(crypto_mode[0]); i++) {
      for (j = 0; j < sizeof(padding_mode) / sizeof(padding_mode[0]); j++) {
        ...
        if (ctx.aes_context.cipher_info->iv_size != 0 &&
            uv_aes_set_iv(&ctx, iv, 0, ctx.aes_context.cipher_info->iv_size) != 0) {
          goto testfail;
        }
